...
namespace DBCourseClients
{
    public partial class UserInfo : Form
    {
        ...
        public UserInfo(OleDbConnection cn)
        {
            InitializeComponent();
            this.cn = cn;
            this.username = Program.username;
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            if (!isChangePsw)
            {
                ...
                if (!isEmpl)
                {
                    ...
                    OleDbCommand cmd = new OleDbCommand("UPDATE Clients SET fio = ? WHERE phone = ?", cn);
                    cmd.Parameters.Add("@p1", OleDbType.VarChar, 50);
                    cmd.Parameters.Add("@p2", OleDbType.VarChar, 11);
                    cmd.Parameters[0].Value = txt_f.Text + " " + txt_i.Text + " " + txt_o.Text;
                    cmd.Parameters[1].Value = txt_phone.Text;
                    try
                    {
                        upd.ExecuteNonQuery();
                        cmd.ExecuteNonQuery();
